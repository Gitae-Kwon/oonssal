...
coin_df = st.cache_data(load_coin_data)()
pay_df = st.cache_data(load_payment_data)()
...
event_input = st.date_input("이벤트 가능성 있는 날짜 선택", value=None, format="YYYY-MM-DD")
if st.button("이벤트 적용") and event_input:
    ...
    if event_input in pay_fut7['ds'].dt.date.tolist(): st.success(f"🚀 {event_input}은 결제 예측 기간에 포함됩니다.")
    if event_input in coin_fut7['ds'].dt.date.tolist(): st.success(f"🚀 {event_input}은 코인 예측 기간에 포함됩니다.")
    if event_input not in pay_fut7['ds'].dt.date.tolist() and event_input not in coin_fut7['ds'].dt.date.tolist():
